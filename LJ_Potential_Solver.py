...
def create_separation_array(particles, box_size, particle_number):
    ...
    for i in range(particle_number):
        for j in range(0,i):
            #Zero alon diagonals
            if i !=j:
                separation = Particle3D.vector_separation(particles[i].position, particles[j].position)
                #Apply MIC
...
def calculate_force(separation, direction_vector):

    return 48.0*((separation**-14.0)-0.5*(separation**-8.0)) * direction_vector


"""Calculate total force on each particle"""
def create_force_array(separation_array, particles, cut_distance, particle_number):
...
def calculate_potential(separation):

    return 4.0*((separation**-12.0)-(separation**-6.0))

"""Calculate total potential energy of the simulation"""
def total_potential(separation_array, particle_number, cut_distance):
...
def total_kinetic(particles, particle_number):
...
def mean_square_displacement(initial_positions, particles, particle_number, box_size):
    ...
    for i, particle in enumerate(particles):
        travel_vector = Particle3D.vector_separation(particle.position, initial_positions[i])
...
"""Write .xyz file"""
def write_traj(particles, particle_number, point, out_file_handle):
...
def velocity_verlet(particles, dt, box_size, particle_number, force_array, cut_distance):
    ...
    #Update separations
    separation_array = create_separation_array(particles, box_size, particle_number)

    #Update forces
    new_force_array = create_force_array(separation_array, particles, cut_distance, particle_number)
...
def main():
    ...
    particles =[Particle3D.generate_particle(i) for i in range(particle_number)]
    ...
    force_array = create_force_array(separation_array, particles, cut_distance, particle_number)
    ...
    msd = mean_square_displacement(initial_positions, particles, particle_number, box_size)
    ...
    for n in range(step_number):
        ...
        #Velocity verlet update
        particles, force_array, separation_array = velocity_verlet(particles, dt, box_size, particle_number, force_array, cut_distance)
